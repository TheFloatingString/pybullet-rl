...
class RobotSimulation(gym.Env):
    ...
    def __init__(self, video_path="simulation.mp4", record_video=True):
        ...
        self.robot_id = p.loadURDF(
            "kuka_iiwa/model.urdf", basePosition=[1.4, -0.2, 0.6], useFixedBase=True
        )
        ...
        if self.record_video:
            self.vid_writer = imageio_ffmpeg.write_frames(
                video_path, (self.cam_width, self.cam_height), fps=30
            )
    ...
    def _capture_frame(self):
        ...
        proj_matrix = p.computeProjectionMatrixFOV(
            self.cam_fov, self.cam_width / self.cam_height, 0.01, 100
        )
